...
class Mahasiswa extends CI_Controller {
	...
	public function index($rowno=0){
		...
		$data['title'] = 'Daftar Mahasiswa '.$infopt->nama_info_pt;
		$data['menu'] = 'menu';
		...
		$data['detail'] = $getuser;
		$data['page'] = 'admin/mahasiswa/index-v';
		$search_text = "";
		if($post == TRUE ){
			$search_text = $post;
			...
			if($this->session->userdata('nama_program_studi') != NULL){
				$post['nama_program_studi'] = $this->session->userdata('nama_program_studi');
			}
			if($this->session->userdata('id_periode_masuk') != NULL){
				$post['id_periode_masuk'] = $this->session->userdata('id_periode_masuk');
			}
			$search_text = $post;
		}
	    // Row per page
		...
		$allcount = $this->Mahasiswa_m->getrecordCount($search_text);
	    // Get records
		$users_record = $this->Mahasiswa_m->getData($rowno,$rowperpage,$search_text);
